...
// 8 blocks).
struct sz_info {
  Bd_list free;       // 空闲块链表的链表
  char *xor_alloc;    // 对于兄弟块B1和B2，记录B1_is_free XOR B2_is_free
  char *split;        // 记录当前块是否已被分裂
};
...
// 用于xor_alloc：若对应位置的值原本为1，则设为0；若对应位置的值原本为0，则设为1
...
bd_malloc(uint64 nbytes)
{
  ...
  bit_xor(bd_sizes[k].xor_alloc, blk_index(k, p)/2);
  for(; k > fk; k--) {
    // split a block at size k and mark one half allocated at size k-1
    // and put the buddy on the free list at size k-1
    char *q = p + BLK_SIZE(k-1);   // p's buddy
    bit_set(bd_sizes[k].split, blk_index(k, p));
    bit_xor(bd_sizes[k-1].xor_alloc, blk_index(k-1, p)/2);
...
bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    bit_xor(bd_sizes[k].xor_alloc, bi/2);  // free p at size k
...
bd_mark(void *start, void *stop)
{
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    for(; bi < bj; bi++) {
      ...
      bit_xor(bd_sizes[k].xor_alloc, bi/2);
    }
  }
}
...
bd_initfree_pair(int k, int bi, int isleft) {
  ...
  int free = 0;
  int p; 
  if(bit_isset(bd_sizes[k].xor_alloc, buddy/2)) {
    // one of the pair is free
    // 元数据信息的分配是从左到右，
    if (isleft)
    {
      // 如果头部分配时出现空闲，那么一定是右手块（奇数编号）加入空闲列表。
      p = (bi % 2 != 0) ? bi : buddy;
    }
    else
    {
      // 同理，尾部分配时，出现空闲一定是左手块（偶数编号）加入空闲列表
      p = (bi % 2 == 0) ? bi : buddy;
    }
    free = BLK_SIZE(k);
    lst_push(&bd_sizes[k].free, addr(k, p));
  }
  return free;
}
...
bd_init(void *base, void *end) {
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    sz = sizeof(char)* ROUNDUP(NBLK(k), 16)/16;   // 将分配给struct sz_info的alloc空间大小除以2
    bd_sizes[k].xor_alloc = p;
    memset(bd_sizes[k].xor_alloc, 0, sz);         // 将相应的内存清0
    p += sz;
  }
